...
-- -----------------------------------------------------
-- Table `Taskify`.`Wiadomosci`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Taskify`.`Wiadomosci` (
  `wiadomosc_id` INT NOT NULL AUTO_INCREMENT,
  `tresc` VARCHAR(255) NOT NULL,
  `uzytkownik_id` INT NOT NULL,
  `projekt_id` INT NOT NULL,
  PRIMARY KEY (`wiadomosc_id`),
  INDEX `fk_Wiadomosci_Uzytkownik1_idx` (`uzytkownik_id` ASC) VISIBLE,
  INDEX `fk_Wiadomosci_Projekty1_idx` (`projekt_id` ASC) VISIBLE,
  CONSTRAINT `fk_Wiadomosci_Uzytkownik1`
    FOREIGN KEY (`uzytkownik_id`)
    REFERENCES `Taskify`.`Uzytkownik` (`uzytkownik_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Wiadomosci_Projekty1`
    FOREIGN KEY (`projekt_id`)
    REFERENCES `Taskify`.`Projekty` (`projekt_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `Taskify`$$
CREATE PROCEDURE `raport2` (IN uzytkownik_id INT)
BEGIN
    SELECT 
        COUNT(DISTINCT p.projekt_id) AS 'ilosc_projektow',
        COUNT(DISTINCT CASE WHEN z.status = 'Do zrobienia' THEN z.zadanie_id END) AS 'ilosc_do_zrobienia',
        COUNT(DISTINCT CASE WHEN z.status = 'Trwajace' THEN z.zadanie_id END) AS 'ilosc_trwajacych',
        COUNT(DISTINCT CASE WHEN z.status = 'Zrobione' THEN z.zadanie_id END) AS 'ilosc_zakonczonych',
        COUNT(DISTINCT CASE WHEN z.status = 'Do zrobienia' THEN za.zalacznik_id END) AS 'ilosc_zalacznikow_do_zrobienia',
        COUNT(DISTINCT CASE WHEN z.status = 'Trwajace' THEN za.zalacznik_id END) AS 'ilosc_zalacznikow_trwajace',
        COUNT(DISTINCT CASE WHEN z.status = 'Zrobione' THEN za.zalacznik_id END) AS 'ilosc_zalacznikow_zakonczone',
        COUNT(DISTINCT CASE WHEN z.status = 'Do zrobienia' THEN k.komentarz_id END) AS 'ilosc_komentarzy_do_zrobienia',
        COUNT(DISTINCT CASE WHEN z.status = 'Trwajace' THEN k.komentarz_id END) AS 'ilosc_komentarzy_trwajace',
        COUNT(DISTINCT CASE WHEN z.status = 'Zrobione' THEN k.komentarz_id END) AS 'ilosc_komentarzy_zakonczone'
    FROM 
        projekty p
    LEFT JOIN 
        zadania z ON p.projekt_id = z.projekt_id
    LEFT JOIN 
        komentarze k ON z.zadanie_id = k.zadanie_id
    LEFT JOIN 
        zalaczniki za ON z.zadanie_id = za.zadanie_id
    INNER JOIN 
        projekty_uzytkownik pu ON p.projekt_id = pu.projekt_id
    WHERE 
        pu.uzytkownik_id = uzytkownik_id;
END$$
